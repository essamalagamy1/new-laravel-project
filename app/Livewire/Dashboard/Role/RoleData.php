...
class RoleData extends Component
{
    ...
    {
        $this->all_roles = Role::when(! auth()->user()->hasRole('superadmin'), fn (Builder $query) => $query->whereNot('name', 'superadmin')
            ->where('is_main', false))
            ->get(['id', 'name'])->toArray();
    ...
    public function render(): View
    {
        $data['roles'] = Role::when($this->search_role_id, fn (Builder $query) => $query->where('id', $this->search_role_id))
            ->when(! auth()->user()->hasRole('superadmin'), fn (Builder $query) => $query->whereNot('name', 'superadmin')
                ->where('is_main', false))
            ->with('permissions')
