	...
	<x-dropdown>
		<x-slot:trigger>
			<div class="indicator relative">
				<x-button icon="fas.bell"  class="btn-ghost btn-sm btn-circle" responsive/>
				...
				@forelse($this->notifications as $notification)
					<div class="block border-b border-base-200 rounded-lg p-2 hover:bg-base-200 transition mb-1 {{ $notification->read_at ?? 'bg-base-300' }}">
						<a class="cursor-pointer" wire:click="readNotification('{{ $notification->id }}')">
							<div class="font-bold">{{ \Illuminate\Support\Str::limit($notification->data['title'][app()->getLocale()], 50) }}</div>
							<div class="text-sm">{{ \Illuminate\Support\Str::limit($notification->data['body'][app()->getLocale()], 50) }}</div>
						</a>
